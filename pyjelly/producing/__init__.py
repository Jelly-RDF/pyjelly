...
from pyjelly.producing.encoders import (
    Slot,
    TermEncoder,
    encode_quad,
    encode_triple,
...
class Stream:
    ...
    def to_triples(
        self,
        triples: Iterable[Iterable[object]],
    ...
    def to_quads(
        self,
        quads: Iterable[Iterable[object]],
    ) -> Generator[jelly.RdfStreamFrame]:
        for quad in quads:
            if frame := self.quad(quad):
                yield frame
    ...
    def graph(
        ...
        yield from self.to_triples(graph)
